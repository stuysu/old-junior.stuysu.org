async function getSheets(keyword, addOn = "&any=") {
  let url = "/api/sheets?";
  if (keyword !== undefined && keyword !== "") url += addOn + keyword;
...
async function fillSearches(firstLoad = false) {
  let result = await getSheets(getSearchbarText(), getQuery());

  let nClass = !firstLoad ? "no-fade-in" : "";
  let html =
    result.length != 0 ? getSheetsHtml(result, nClass) : getDefaultHtml(nClass);
