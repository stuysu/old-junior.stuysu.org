...
class Time {
  ...
  constructor(hours, minutes) {
    ...
    this.hours = validateRange(hours, 0, 23, "hours");

    this.minutes = validateRange(minutes, 0, 59, "minutes");
  }
  ...
  toString() {
    return `${this.get12Hour()}:${leadingZero(
      this.minutes
    )} ${this.getMeridiem()}`;
  }
...
class Period {
  constructor(first, second, payload = {}) {
    if (first.compare(second) !== -1) {
      throw new Error(
        `${first} does not come before ${second} in creating period ${name}`
      );
    }
