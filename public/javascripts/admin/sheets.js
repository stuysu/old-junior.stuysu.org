...
async function removeSheet(id) {
  ...
  if (response.id) {
    if (response.deleted === false) {
      alertManager.addAlert(
        "Failure",
        "did not find a study sheet to delete",
        "secondary"
      );
    } else {
...
async function addSheet() {
  ...
  if (response.created) {
    ...
    for (const type in tmp) tmp[type].value = "";
...
async function removeKeyword(keywordId) {
  ...
  if (response.id) {
    if (response.deleted === false) {
      alertManager.addAlert(
        "Failure",
        "did not find a keyword to delete",
        "secondary"
      );
    } else {
...
async function addKeyword(sheetId) {
  const keywordText = document.getElementById(
    sheetId + "-add-keyword-text"
  ).value;
  ...
  if (response.error) {
    ...
    if (response.created) {
      alertManager.addAlert(
        "Success",
        `added keyword ${response.keyword}`,
        "success"
      );
    } else {
      alertManager.addAlert(
        "Failure",
        "didn't create study sheet keyword",
        "warning"
      );
    }
  }
