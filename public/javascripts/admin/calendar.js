...
async function getExistingDate(date) {
  let request = await sfetch(
    `/api/calendar?day=${date.day}&month=${date.month}&year=${date.year}`
  );
...
async function updateCalendarForm() {
  ...
  if (existingDate != null) {
    ...
    calendarInputs.dayLetter.selectedIndex = getSelectIndex(
      calendarInputs.dayLetter,
      existingDate.dayLetter
    );
    ...
    calendarInputs.dayType.selectedIndex = getSelectIndex(
      calendarInputs.dayType,
      existingDate.dayType
    );
...
async function saveDate() {
  if (!isCalendarInputValid()) {
    alertManager.addAlert(
      "Failure",
      "not all date inputs are filled out, cannot make a new date entry",
      "warning"
    );
    return;
  }
  ...
  if (response.updated) {
    alertManager.addAlert(
      "Success",
      "updated existing date with new information"
    );
  } else if (response.id) {
...
async function clearDate() {
  ...
  if (existingDate != null) {
    ...
    if (response.deleted) {
      alertManager.addAlert(
        "Success",
        "Selected date cleared from the database"
      );
      return;
    }
  }
  ...
  alertManager.addAlert(
    "Failure",
    "No date to clear from database, or database error",
    "warning"
  );
}
